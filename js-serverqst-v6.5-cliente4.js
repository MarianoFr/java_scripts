...
6.5 23/05/2022
  ...
  Ya no se pregunta por updated modem
*/ 
...
{
  if(topic.includes('cliente4'))
  {
    var msg = message.toString();
    console.log("Mensaje recibido desde -> " + topic + " Mensaje -> " + msg);
    
    if(topic.includes('cliente4/report'))
    {
      if(msg == "")
      {
      ...
      else
      {
        ...
        if(type == "ACK")
        {
          var query_ack = "UPDATE `insoltechv14`.`reconfig_time` " +
    ...
    else if(topic.includes('cliente4/reconfig'))
    {
      if(msg == "")
      {
    ...
    else if(!topic.includes('report'))
    {
        ...
        if(globalTag!=tag)
        {
          ...
          {  
            con.query(queryInfo, function (err, result, fields) 
            {
                ...
                {                
                    ...
                    { //Se inserta fila info
                        if((parseFloat(GPSlat) == 0) || (parseFloat(GPSlng) == 0))
                        {
                            ...
                            query1 = "INSERT INTO `insoltechv14`.`info` (`modem`, `nroInfo`,`touridx`, `infono`, `FechaHora`,"+
                        ...
                        else
                        {
                            AccuracyGlobal = 20.0;
                            //Se inserta fila info
                            query1 = "INSERT INTO `insoltechv14`.`info` (`modem`, `nroInfo`,`touridx`, `infono`, `FechaHora`, `GPSlat`," 
                            ...
                            //Se inserta fila gpspos con ícono de info
                            query2 = "INSERT INTO `insoltechv14`.`gpspos` (  `modem`, `GPSlat`, `GPSlng`, `Velocidad`, `Kilometros`, `Accuracy`, `tag`, `mapIcon`, "
                    ...
                    { //Se inserta fila info
                        ...
                        //Se inserta fila gpspos con ícono de info
                        query2 = "INSERT INTO `insoltechv14`.`gpspos` (  `modem`, `GPSlat`, `GPSlng`, `Velocidad`, `Kilometros`, `Accuracy`, `gpsTime`, `tag`, `mapIcon`, "
                        +"`IconName`, `patente`) VALUES ('" + modem + "', '" + GPSlat +"', '" + GPSlng + "', '" + Velocidad + "', '" 
                ...
                {
                    var queryError = "INSERT INTO `insoltechv14`.`errores` (`mensaje`) VALUES ('" + msg + "');";
      ...
      if (topic == "cliente4/recorrido")
      {
        ...
        if(globalTag!=tag)
        {
          ...
          if(gpsFechaHora=="2021-01-01 08:00:00")
          {
            query1 = "INSERT INTO `insoltechv14`.`recorrido` (`modem`,`nroRecorrido`,`touridx`, `nroCamion`,`nroCuenta`, "
              + "`nroAcoplado`,`remito`,`recorrido`,`FechaHoraInicio`, `FechaHoraFin`,`cantidadTotal`,`contadorMuestras`, "
              + "`tag`, `patente`, `chofer`) VALUES ('"
              + modem + "', '" + nroRecorrido +"', '" + touridx + "', '" + nroCamion + "', '" + nroCuenta + "', '"
              + nroAcoplado + "', '" + remito + "', '" + recorrido + "', '" + fechaHoraInicio + "', '"+ fechaHoraFin +"', '"
              + cantidadTotal + "', '" + contadorMuestras + "', '" + tag + "', '" + rx_patente + "', '" + chofer +  "');";
          }
          else
          {
            query1 = "INSERT INTO `insoltechv14`.`recorrido` (`modem`,`nroRecorrido`,`touridx`, `nroCamion`,`nroCuenta`, "
              + "`nroAcoplado`,`remito`,`recorrido`,`FechaHoraInicio`, `FechaHoraFin`,`cantidadTotal`,`contadorMuestras`, "
              + "`GPSlat`, `GPSlng`, `Velocidad`, `Kilometros`, `gpsTime`, `tag`, `patente`, `chofer`) VALUES ('"
              + modem + "', '" + nroRecorrido +"', '" + touridx + "', '" + nroCamion + "', '" + nroCuenta + "', '"
              + nroAcoplado + "', '" + remito + "', '" + recorrido + "', '" + fechaHoraInicio + "', '"+ fechaHoraFin +"', '"
              + cantidadTotal + "', '" + contadorMuestras + "', '"+ GPSlat +"', '" + GPSlng+ "', '" + Velocidad + "', '"
              + Kilometros + "', '" + gpsFechaHora + "', '"+ tag + "', '" + rx_patente + "', '" + chofer +  "');";		
          }
          console.log(query1);
          try
          {
      ...
      if (topic == "cliente4/tambo")
      {
        ...
        if(globalTag!=tag)
        {
          ...
          if(gpsFechaHora=="2021-01-01 08:00:00")
          {
            if((parseFloat(GPSlat) == 0) || (parseFloat(GPSlng) == 0))
            {
              query1 = "INSERT INTO `insoltechv14`.`tambo` ( `modem`, `nroTambo`, `touridx`, `remito`, `observaciones`, `codigoTambero`, `cantidadTotal`, `compa`,"
                + " `cantidadA`, `compb`, `cantidadB`, `compc`, `cantidadC`, `tempaver`, `tempmin`, `tempmax`, `status`, `LTM`, `desaInicial`, `desaFinal`,"
                + " `constanteE`, `constanteC`, `SMP`, `FechaHoraInicio`, `FechaHoraFin`, `tag`, `patente`)"
              ...
              query2 = "";
              //Actualiza uTambos
              query3 = "REPLACE INTO `insoltechv14`.`uTambos` (  `modem`,`tag`, `mapIcon`, `nroTambo`, `cantidad`, `patente`) VALUES ('" 
                + modem + "', '" + tag + "', '" + tambo_icon + "', '" + codigoTambero
                + "', '" + cantidadTotal + "', '" + rx_patente + "');";
            }
            else
            {
              AccuracyGlobal = 20;
              ...
              query2 = "INSERT INTO `insoltechv14`.`gpspos` (  `modem`, `GPSlat`, `GPSlng`, `Velocidad`, `Kilometros`, `Accuracy`, `tag`, `mapIcon`, "
                ...
                //Actualiza uTambos
              query3 = "REPLACE INTO `insoltechv14`.`uTambos` (  `modem`, `GPSlat`, `GPSlng`,`tag`, `mapIcon`, `nroTambo`, `cantidad`, `patente`) VALUES ('" 
                + modem + "', '" + GPSlat +"', '" + GPSlng + "', '" + tag + "', '" + tambo_icon + "', '" + codigoTambero
                + "', '" + cantidadTotal + "', '" + rx_patente + "');";
            }
          }
          else
          {
            ...
            //Se inserta fila gpspos con ícono de tambo
            query2 = "INSERT INTO `insoltechv14`.`gpspos` (  `modem`, `GPSlat`, `GPSlng`, `Velocidad`, `Kilometros`, `Accuracy`, `tag`, `gpsTime`, `mapIcon`, "
              +"`IconName`, `patente`) VALUES ('" + modem + "', '" + GPSlat +"', '" + GPSlng + "', '" + Velocidad + "', '" 
              + Kilometros + "', '" + AccuracyGlobal + "', '" + tag + "', '" + gpsFechaHora + "', '" + tambo_icon + "', 'Tambo', '" + rx_patente + "');";

            //Actualiza uTambos
            query3 = "REPLACE INTO `insoltechv14`.`uTambos` (  `modem`, `GPSlat`, `GPSlng`,`tag`, `gpsTime`, `mapIcon`, `nroTambo`, `cantidad`, `patente`) VALUES ('" 
              + modem + "', '" + GPSlat +"', '" + GPSlng + "', '" + tag + "', '" + gpsFechaHora + "', '" + tambo_icon + "', '" + codigoTambero
              + "', '" + cantidadTotal + "', '" + rx_patente + "');";
          }
          try
          {
      ...
      if (topic == "cliente4/cip")
      {
        ...
        if(globalTag!=tag)
        {
          ...
          { //Se inserta fila cip
            if((parseFloat(GPSlat) == 0) || (parseFloat(GPSlng) == 0))
            {
              query1 = "INSERT INTO `insoltechv14`.`cip` (  `modem`, `nroCip`, `touridx`, `nroCamion`, `tempaver`, `tempmin`, `tempmax`,"
            ...
            else
            {
              ...
              AccuracyGlobal = 20;
            //Se inserta fila gpspos con ícono de cip
              query2 = "INSERT INTO `insoltechv14`.`gpspos` (  `modem`, `GPSlat`, `GPSlng`, `Velocidad`, `Kilometros`, `Accuracy`, `tag`, "
          ...
          { //Se inserta fila cip
            ...
            //Se inserta fila gpspos con ícono de cip
            query2 = "INSERT INTO `insoltechv14`.`gpspos` (  `modem`, `GPSlat`, `GPSlng`, `Velocidad`, `Kilometros`, `Accuracy`, `tag`, `gpsTime`, "
              + "`mapIcon`, `IconName`, `patente`) VALUES ('" + modem + "', '" + GPSlat +"', '" + GPSlng + "', '" + Velocidad
      ...
      if (topic == "cliente4/trasvase")
      {
        ...
        if(globalTag!=tag)
        {
          ...
          { //Se inserta fila trasvase
            if((parseFloat(GPSlat) == 0) || (parseFloat(GPSlng) == 0))
            {
              query1 = "INSERT INTO `insoltechv14`.`trasvase` (  `modem`, `nroTrasvase`, `touridx`, `Sourcecomp`, `destcomp`, `trailerno`,"
            ...
            else
            {
              query1 = "INSERT INTO `insoltechv14`.`trasvase` (  `modem`, `nroTrasvase`, `touridx`, `Sourcecomp`, `destcomp`, `trailerno`,"
                ...
                AccuracyGlobal = 20;
              //Se inserta fila gpspos con ícono de trasvase
              query2 = "INSERT INTO `insoltechv14`.`gpspos` (  `modem`, `Velocidad`, `Kilometros`, `Accuracy`, `GPSlat`, `GPSlng`, `tag`,"
      ...
      if (topic == "cliente4/gps") 
      {
        ...
        if(globalTag!=tag)
        {
          ...
          con.query(query0, function (err, result, fields)
          {
            ...
            var query2 = "INSERT INTO `insoltechv14`.`gpspos` (  `modem`, `GPSlat`, `GPSlng`, `Velocidad`, `Kilometros`, `Accuracy`, `tag`,"
            ...
            con.query(query2, function (err, result, fields) 
            {
              if (err) throw err;
              console.log("Fila 'gpspos' insertada correctamente");
              var query = "REPLACE INTO `insoltechv14`.`uPos` (`modem`, `GPSlat`, `GPSlng`, `Velocidad`, `Kilometros`, `tag`, `gpsTime`, "
      ...
      if (topic == "cliente4/fails")
      {
        ...
        var query = "INSERT `insoltechv14`.`alertas` SET `modem` = '" + modem + "', `SD` = '" + sd
        ...
        if(gpsFechaHora=="2021-01-01 08:00:00")
        {
          query = "INSERT `insoltechv14`.`alertas` SET `modem` = '" + modem + "', `SD` = '" + sd
          + "', `Q` = '" + q + "', `GPS` = '" + gps + "', `resets` = '" + nro_resets + "', `GPSlat` = '"
      ...
      if (topic == "cliente4/parametros")
      {
        ...
        if(globalTag!=tag)
        {
          ...
          if(gpsFechaHora=="2021-01-01 08:00:00")
          {
            query_param = "INSERT INTO `insoltechv14`.`param_report` ( `modem`, `uNbr`, `trailN`," +
      ...
      else
      {
        try
        {
          if(update_sent!=String(result[0]['update']))
          {
            ...
            var topic = "cliente4/reconfig/" + String(result[0]['modem']);
      ...
      else
      {
        try
        {
          ...
          if(last_sol != String(result[0]['solicitud']))
          {
            ...
            sol_topic = "cliente4/report/" + result[0]["modem"];
            sol_msg = '{"msg":"REPORT"}';
