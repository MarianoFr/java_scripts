/*
6.1
  Version para clientes de planta, sin GPS
*/ 
...
mqtt_client.on('connect', function ()
{
  ...
  //client.subscribe('caudalimetro/#', function (err) {
...
{
  if(topic.includes('cliente3'))
  {
    var msg = message.toString();
    console.log("Mensaje recibido desde -> " + topic + " Mensaje -> " + msg);
    
    if(topic.includes('cliente3/report'))
    {
      if(msg == "")
      {
      ...
      else
      {
        ...
        if(type == "ACK")
        {
          var query_ack = "UPDATE `insoltechv13`.`reconfig_time` " +
    ...
    else if(topic.includes('cliente3/reconfig'))
    {
      if(msg == "")
      {
    ...
    // CREATEQUERY TO BRING INFO TEXT
        ...
        con.query(queryInfo, function (err, result, fields) 
        {
          ...
          console.log(texto);
          query1 = "INSERT INTO `insoltechv13`.`info` (`modem`, `nroInfo`,`touridx`, `infono`, `FechaHora`,"+
          " `tag`) VALUES ('" + modem + "', '" + nroInfo + "', '"
          + touridx + "', '" + texto +"', '" + fechaHora + "', '" + tag +  "');";
          console.log(query1);
      ...
      if (topic == "cliente3/recorrido")
      {
        var modem = json_parser['md'];
        var nroRecorrido = json_parser['fld']['nR'];
        var touridx = json_parser['fld']['tid'];
        var nroCamion = json_parser['fld']['nCm'];
        var nroCuenta = json_parser['fld']['nCt'];  
        var nroAcoplado = json_parser['fld']['nAc'];
        ...
        var tag = json_parser['tag'];
        query1 = "INSERT INTO `insoltechv13`.`recorrido` (`modem`,`nroRecorrido`,`touridx`, `nroCamion`,`nroCuenta`, "
            ...
            + cantidadTotal + "', '" + contadorMuestras + "', '" + tag +  "');";        
        con.query(query1, function (err, result, fields) 
        {
          if (err) throw err;
          console.log("Fila 'recorrido' insertada correctamente");
        });
      }//Ok
      if (topic == "cliente3/tambo")
      {
        var modem = json_parser['md'];
        ...
        var tempAver = json_parser['fld']['tA'];
        ...
        var constanteC = json_parser['fld']['ctC'];
        var sampleID = json_parser['fld']['smp'];
        var fechaHoraInicio = json_parser['fld']['FHI'];
        var fechaHoraFin = json_parser['fld']['FHF'];
        var tag = json_parser['tag'];
        query1 = "INSERT INTO `insoltechv13`.`tambo` ( `modem`, `nroTambo`, `touridx`, `remito`, `codigoTambero`, `cantidadTotal`, `compa`,"
              ...
              + "VALUES ('" + modem + "', '" + nroTambo +"', '" + touridx + "', '" + remito + "', '" + codigoTambero + "', '" + cantidadTotal + "', '"
              + compA + "', '" + cantidadA + "', '" + compB + "', '" + cantidadB + "', '" + compC + "', '" + cantidadC + "', '" 
              + tempAver + "', '" + tempMin + "', '" + tempMax + "', '" + status + "', '" + litrosTmuestra + "', '" + desaInicial + "', '" + desaFinal + "', '" 
              + constanteE + "', '" + constanteC + "', '" + sampleID + "', '" + fechaHoraInicio + "', '" + fechaHoraFin  + "', '" + tag + "');";        
        con.query(query1, function (err, result, fields) 
        {
          if (err) throw err;
      ...
      if (topic == "cliente3/cip")
      {
        ...
        var fechaHoraFin = json_parser['fld']['FHF'];
        var tag = json_parser['tag'];
        query1 = "INSERT INTO `insoltechv13`.`cip` (  `modem`, `nroCip`, `touridx`, `nroCamion`, `tempaver`, `tempmin`, `tempmax`,"
            + " `totalLitros`, `FechaHoraInicio`, `FechaHoraFin`, `tag`)"
            +" VALUES ('" + modem + "', '" + nroCip +"', '" + touridx + "', '" + nroCamion + "', '" + tempAver + "', '" + tempMin + "', '"
            + tempMax + "', '" + totalLitros + "', '" + fechaHoraInicio + "', '"+ fechaHoraFin  + "', '" + tag + "');";
        con.query(query1, function (err, result, fields) 
        {
          if (err) throw err;
      ...
      if (topic == "cliente3/trasvase")
      {
        ...
        var fechaHora = json_parser['fld']['FH'];
        var tag = json_parser['tag'];
        query1 = "INSERT INTO `insoltechv13`.`trasvase` (  `modem`, `nroTrasvase`, `touridx`, `Sourcecomp`, `destcomp`, `trailerno`,"
            + " `cantidad`, `FechaHora`, `tag`) VALUES ('" + modem + "', '"
      ...
      if (topic == "cliente3/fails")
      {
        ...
        var query = "INSERT `insoltechv13`.`alertas` SET `modem` = '" + modem + "', `SD` = '" + sd
        + "', `Q` = '" + q + "', `resets` = '" + nro_resets + "', `tag` = '" + tag + "';";
      ...
      if (topic == "cliente3/parametros")
      {
        ...
        var wssid = json_parser['fld']['wssid'];
        var wpass = json_parser['fld']['wpass'];
        var serialq_nr = json_parser['fld']['sQNR'];
        var q_to = json_parser['fld']['sQTO'];
        var backoff = json_parser['fld']['backoff'];
        var tag = json_parser['tag'];
        var query_param;
        query_param = "INSERT INTO `insoltechv13`.`param_report` ( `modem`, `uNbr`, `trailN`," +
            ...
            " `tag`, `wssid`, `wpass`, `sQNR`, `sQTO`, `backoff`, `gpsTime`)" + 
            ...
            cipMxT + "', '" + tag + "', '" + wssid + "', '" + wpass + "', '" + 
            serialq_nr + "', '" + q_to + "', '" + backoff + "', CURRENT_TIMESTAMP);";
      ...
      else
      {
        try
        {
          if(update_sent!=String(result[0]['update']))
          {
            ...
            var topic = "cliente3/reconfig/" + String(result[0]['modem']);
      ...
      else
      {
        try
        {
          ...
          if(last_sol != String(result[0]['solicitud']))
          {
            ...
            sol_topic = "cliente3/report/" + result[0]["modem"];
            sol_msg = '{"msg":"REPORT"}';
